...
fetch_thumb() {
    ...
    playerctl -p spotify_player status &>/dev/null || { cleanup; exit 1; }

    # Get album art URL from Spotify
    artUrl=$(playerctl -p spotify_player metadata --format '{{mpris:artUrl}}' 2>/dev/null)
    [[ -z "$artUrl" ]] && exit 1
...
playerctl -p spotify_player status &>/dev/null && {
    playerctl -p spotify_player metadata --format '{{title}}  {{artist}}' && fetch_thumb
} || cleanup &
